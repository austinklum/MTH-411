	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\begin{enumerate}
				...
				$<\Q,*>$ onto $<\Q,+>$
				As $ \phi $ is a bijection, we need to define $ * $ such that $ \phi $ is a homomorphism. That is $ \phi(x*y)=\phi(x)+\phi(y) $ for $ x,y \in\Q $
				Let $ * $ be defined as $ x*y = x+y-\frac{1}{3}$ for $ x,y\in\Q $. Using this definition, let $ x,y\in\Q $
				\[\phi(x)+\phi(y)=(3x-1)+(3y-1)=3x+3y-2\] and
				\begin{align*}
				\phi(x * y) &= 3(x*y)-1\\
				&= 3(x+y-\frac{1}{3})-1 \\
				&= 3x+3y-1-1 \\
				...
				Therefore, $ \phi $ is an isomorphism.\\
				\\
				As 0 is the identity element of $\Q$  under addition, and $ \phi $ is an isomorphism, $ \phi $ must map some element, $i$ to the identity element of $ \Q $ under addition.
				\begin{align*}
				\phi(i) &= 0 \\
				3i-1 &= 0 \\
				i &= \frac{1}{3} 
				...
				Therefore, the identity element of $ \Q $ under $ * $ is $\frac{1}{3}$
			\end{enumerate}
			...
			\item[3.21] A function $\phi:S \rightarrow S'$ is an \textit{isomorphism} if and only if $\phi(a* b) = \phi(a) * \phi(b)$\\
			
			Let $<S,*> $ and $<S',*'>$ be binary algebraic structures. A function $ \phi: S \rightarrow \S' $ is an isomorphism if and only if $ \phi $ is injective, surjective, and satisfies $ \forall a,b\in S, \phi(a*b) = a*'b $
